...
// 1) As the array is already sorted, start traversing the array from the last using a for loop and check whether the element is present or not.
...
int lasttOccurence2(vector<int> &arr, int n, int target){
	int s = 0;
	int e = arr.size() - 1;
	int ans = -1;
...
int main()
{
    ...
    int target;
    cin>>target;
	cout<<lasttOccurence2(arr, n, target);
    return 0;
}